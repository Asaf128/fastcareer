import { consumePeriodUnique, refundPeriodUnique, peekPeriodRemaining } from '@/lib/usage'
 ...
 * → Gratis-Kontingent (2 alle 7 Tage, Redis) → gekaufte Credits (Supabase)
 * → Limit erreicht. Anonyme Nutzer haben nur das Gratis-Kontingent.
 */
...
export interface QuotaParams {
  ...
  /** Redis-Schlüssel fürs Gratis-Kontingent: User-ID oder `ip:<addr>` */
  userKey: string
...
export async function consumeAiQuota(params: QuotaParams): Promise<QuotaResult> {
  ...
  const free = await consumePeriodUnique(params.feature, params.userKey, params.jobRefnr)
  if (free.allowed) {
    return { allowed: true, source: 'free', remaining: free.remaining, creditCharged: false }
  }
...
/**
 * Verfügbares Gesamt-Kontingent (Gratis + gekaufte Credits) nur LESEN, für
 * die Anzeige, ob ein Panel den Limit-Zustand zeigen muss.
 * null = unbegrenzt (Pro).
 */
export async function peekAiQuota(
  ...
  const free = await peekPeriodRemaining(feature, userKey)
