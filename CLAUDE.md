...
- Kontingent-Kaskade + Credit-Pakete (Pro → 3 Gratis/Tag → gekaufte Credits; Guthaben `credit_balances`, Verbrauchs-Log mit Dedupe `credit_usages`, atomare Postgres-Funktionen `consume_credit`/`refund_credit`): `src/lib/quota.ts` (Kaskade), `src/lib/credits.ts` (RPC-Wrapper); durchgesetzt in `suche/[refnr]/page.tsx` + den KI-Server-Actions. Gutschrift erfolgt künftig über den Stripe-Webhook (Schritt 2), Kauf-UI folgt (Schritt 3)
