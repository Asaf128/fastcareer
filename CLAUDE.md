...
- Freemium-Tageskontingente (3/Tag pro Feature: Zusammenfassung/Match/Anschreiben/CV; anonym per IP als Schnupper-Kontingent, Redis-Tageszähler): `src/lib/usage.ts`, gemeinsamer Upstash-Client `src/lib/redis.ts`, Limit-UI `src/components/jobs/UsageLimit.tsx`; durchgesetzt in `suche/[refnr]/page.tsx` + den KI-Server-Actions
